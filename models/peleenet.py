    ...
    def __init__(self, inp, oup, kernel_size=3, stride=1, pad=1,use_relu = True):
        ...
        if self.use_relu:
            self.convs = nn.Sequential(
                ...
                nn.BatchNorm2d(oup), # nn.BatchNorm2d(oup, eps=0.001, momentum=0.001),
...
class PeleeNet(nn.Module):
    ...
    def load_param_v2(self, model_path):
        param_dict = torch.load(model_path, map_location='cpu')['state_dict']
        model_dict = self.state_dict()
        for i in model_dict:
            map_i = 'module.' + i
            map_i = map_i.replace("module.stages", "module.stage")
...
def peleenet(pretrained = False, **kwargs):
    return PeleeNet(num_classes=1000, num_init_features=32, growthRate=32, nDenseBlocks = [3,4,8,6], bottleneck_width=[1,2,4,4])

if __name__ == '__main__':
    # for peleenet
    model = PeleeNet(num_classes=1000, num_init_features=32, growthRate=32, nDenseBlocks = [3,4,8,6], bottleneck_width=[1,2,4,4])
    model.load_param("model_best.pth.tar")
    
    # for peleenet 1444
    # model = PeleeNet(num_classes=1000, num_init_features=32, growthRate=32, nDenseBlocks = [3, 4, 8, 6], bottleneck_width=[1,4,4,4])
    # model.load_param_v2("model_best_wide_1444.pth.tar")

    # for peleenet 64 
    # model = PeleeNet(num_classes=1000, num_init_features=32, growthRate=32, nDenseBlocks = [3, 4, 6, 8, 6], bottleneck_width=[1,2, 4,4,4])
    # model.load_param("pelee64_71.8.pth.tar")
