...
class LinearBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_shortcut:
            out_1, out_2 = torch.split(out, [self.in_channels, out.shape[1] - self.in_channels], dim=1)
            out_1 += x
...
def rexnet(pretrained = False, **kwargs):
    return ReXNetV1(num_classes=1000, width_mult=1.0)

if __name__ == '__main__':
    model = ReXNetV1(num_classes=1000, width_mult=1.0)
    import time
