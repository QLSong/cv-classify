...
python3 -m torch.distributed.launch --nproc_per_node=1 main.py /workspace/mnt/storage/songqinglong/imagenet/ImageNet-pytorch \
  --model mobilenet_v3_large --img-size 224 \
  -b 256 --lr 1.6e-3 --drop-path 0.1 --apex-amp \
  --token-label \
