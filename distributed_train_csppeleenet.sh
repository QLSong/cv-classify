...
python3 -m torch.distributed.launch --nproc_per_node=2 main.py /workspace/mnt/storage/songqinglong/data/ImageNet-pytorch \
  --model csppeleenet --img-size 224 \
  -b 128 --lr 1.6e-3 --drop-path 0.1 --apex-amp \
  --token-label \
  --token-label-data /workspace/mnt/storage/songqinglong/data/label_top5_train_nfnet